...
def get_shortcut_path():
    ...
    return os.path.join(startup_folder, "Periodic Info Reminder.lnk")
        ...
        if not os.path.exists(shortcut_path):
            try:
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    logger.warning("在开发模式下创建自启快捷方式可能不准确，推荐在打包后使用。")
                    with winshell.shortcut(shortcut_path) as shortcut:
                        shortcut.path = sys.executable.replace("python.exe", "pythonw.exe")
                        shortcut.arguments = f'"{os.path.abspath(sys.argv[0])}"'
                        shortcut.working_directory = os.path.dirname(os.path.abspath(sys.argv[0]))
                        shortcut.description = "Periodic Information Reminder"
